...
static void esp_mem_print_task() {
    ...
    for (int i = 0; i < uxArraySize; i++) {
        ...
        ESP_LOGI(TAG,
                 "%-16s\t%c\t%" PRIu32 "\t%" PRIu32 "\t%" PRIu32 "\t%hd\t%-16" PRIu32 "%-s%%",
                 pxTaskStatusArray[i].pcTaskName,
                 task_status_char[pxTaskStatusArray[i].eCurrentState],
                 ...
                 core_id,
                 ulRunTimeCounte,
                 (ulStatsAsPercentage <= 0) ? "<1" : itoa(ulStatsAsPercentage, precentage_char, 10));
    }
...
void qmsd_debug_heap_diff_print(uint32_t caps, uint8_t refresh) {
    static multi_heap_info_t info_old = {0};

    multi_heap_info_t info_new;
    heap_caps_get_info(&info_new, caps);
    ESP_LOGI(TAG, "------------- ಥ_ಥ Memory ಥ_ಥ ------------- ");
    ESP_LOGI(TAG, "ಥ_ಥ  total_free_bytes -> %d, diff: %" PRId32, info_new.total_free_bytes, (int32_t)(info_new.total_free_bytes - info_old.total_free_bytes));
    ESP_LOGI(TAG, "ಥ_ಥ  total_allocated_bytes -> %d, diff: %" PRId32, info_new.total_allocated_bytes, (int32_t)(info_new.total_allocated_bytes - info_old.total_allocated_bytes));
    ESP_LOGI(TAG, "ಥ_ಥ  largest_free_block -> %d, diff: %" PRId32, info_new.largest_free_block, (int32_t)(info_new.largest_free_block - info_old.largest_free_block));
    ESP_LOGI(TAG, "ಥ_ಥ  minimum_free_bytes -> %d, diff: %" PRId32, info_new.minimum_free_bytes, (int32_t)(info_new.minimum_free_bytes - info_old.minimum_free_bytes));
    ESP_LOGI(TAG, "ಥ_ಥ  allocated_blocks -> %d, diff: %" PRId32, info_new.allocated_blocks, (int32_t)(info_new.allocated_blocks - info_old.allocated_blocks));
    ESP_LOGI(TAG, "ಥ_ಥ  free_blocks -> %d, diff: %" PRId32, info_new.free_blocks, (int32_t)(info_new.free_blocks - info_old.free_blocks));
    ESP_LOGI(TAG, "ಥ_ಥ  total_blocks -> %d, diff: %" PRId32, info_new.total_blocks, (int32_t)(info_new.total_blocks - info_old.total_blocks));
    ESP_LOGI(TAG, "ಥ_ಥ  total_mem -> %d, diff: %" PRId32, info_new.total_free_bytes + info_new.total_allocated_bytes, (int32_t)((info_new.total_free_bytes + info_new.total_allocated_bytes) - (info_old.total_free_bytes + info_old.total_allocated_bytes)));
...
esp_err_t qmsd_thread_create(TaskFunction_t main_func,
                             const char* const name,
                             const uint32_t stack_size,
                             void* const arg,
                             UBaseType_t prio,
                             TaskHandle_t* const p_handle,
                             BaseType_t core_id,
                             uint8_t stack_in_ext) {
...
void qmsd_thread_delete(TaskHandle_t* p_handle) {
    // 只允许删除当前线程自身
    if (p_handle != NULL) {
        ESP_LOGE(TAG, "qmsd_thread_delete only supports deleting current task (p_handle must be NULL)");
        return;
    }
...
#ifdef CONFIG_SPIRAM
    // 获取当前堆栈指针，通过检查堆栈指针地址来判断是否在PSRAM中
    void* current_stack_ptr = __builtin_frame_address(0);

    // 检查当前堆栈指针是否在PSRAM范围内
    bool stack_in_spiram = esp_ptr_external_ram(current_stack_ptr);

    if (stack_in_spiram) {
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 2, 0)
#if (configSUPPORT_STATIC_ALLOCATION == 1 && CONFIG_SPIRAM_ALLOW_STACK_EXTERNAL_MEMORY == 1)
        ESP_LOGI(TAG, "Deleting current task with PSRAM stack using vTaskDeleteWithCaps");
        vTaskDeleteWithCaps(current_task);
#else
        ESP_LOGW(TAG, "PSRAM stack detected but CONFIG_SPIRAM_ALLOW_STACK_EXTERNAL_MEMORY not enabled, using vTaskDelete");
        vTaskDelete(current_task);
#endif
#else
        ESP_LOGW(TAG, "PSRAM stack detected but IDF version < 5.2.0, using vTaskDelete (may cause memory leak)");
        vTaskDelete(current_task);
#endif
    } else {
        ESP_LOGI(TAG, "Deleting current task with internal RAM stack");
        vTaskDelete(current_task);
    }
#else
    ESP_LOGI(TAG, "Deleting current task (SPIRAM not configured)");
    vTaskDelete(current_task);
#endif
}
