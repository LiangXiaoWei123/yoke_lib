...
static const st7789_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    { LCD_CMD_SLPOUT, NULL, 0, 100 }, // Sleep out + 100ms delay
};
...
esp_err_t lcd_new_panel_st7789_spec(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel) {
    ...
    const st7789_vendor_config_t *vendor_config = (const st7789_vendor_config_t *)panel_dev_config->vendor_config;
    if (vendor_config && vendor_config->init_cmds && vendor_config->init_cmds_size) {
        st7789->init_cmds = vendor_config->init_cmds;
        st7789->init_cmds_size = vendor_config->init_cmds_size;
    } else {
        st7789->init_cmds = vendor_specific_init_default;
        st7789->init_cmds_size = sizeof(vendor_specific_init_default) / sizeof(st7789_lcd_init_cmd_t);
    }
...
static esp_err_t panel_st7789_init(esp_lcd_panel_t *panel) {
    st7789_panel_t *st7789 = __containerof(panel, st7789_panel_t, base);
    esp_lcd_panel_io_handle_t io = st7789->io;
    const st7789_lcd_init_cmd_t *init_cmds = st7789->init_cmds;
    uint16_t init_cmds_size = st7789->init_cmds_size;
    ...
    const st7789_lcd_init_cmd_t *init_cmd = NULL;
    for (int i = 0; i < init_cmds_size; i++) {
        if (init_cmds[i].cmd == LCD_CMD_SLPOUT) {
            init_cmd = &init_cmds[i];
            break;
        }
    }
    if (!init_cmd) {
        init_cmd = &vendor_specific_init_default[0];
    }

    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, init_cmd->cmd, init_cmd->data, init_cmd->data_bytes), TAG, "io tx param failed");
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_COLMOD,
                                                  ...
                                                  1),
                        TAG, "io tx param failed");
    ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, ST7789_CMD_RAMCTRL, (uint8_t[]){ st7789->ramctl_val_1, st7789->ramctl_val_2 }, 2), TAG,
                        "io tx param failed");

    for (int i = 0; i < init_cmds_size; i++) {
        if (&init_cmds[i] == init_cmd) {
            continue;
        }
        is_cmd_overwritten = false;
        const uint8_t *data = (const uint8_t *)init_cmds[i].data;
        switch (init_cmds[i].cmd) {
            case LCD_CMD_MADCTL:
                if (data && init_cmds[i].data_bytes > 0) {
                    st7789->madctl_val = data[0];
                }
                is_cmd_overwritten = true;
                break;
            case LCD_CMD_COLMOD:
                if (data && init_cmds[i].data_bytes > 0) {
                    st7789->colmod_val = data[0];
                }
                is_cmd_overwritten = true;
                break;
            case ST7789_CMD_RAMCTRL:
                if (data && init_cmds[i].data_bytes >= 2) {
                    st7789->ramctl_val_1 = data[0];
                    st7789->ramctl_val_2 = data[1];
                }
                is_cmd_overwritten = true;
                break;
            default:
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, init_cmds[i].cmd, init_cmds[i].data, init_cmds[i].data_bytes), TAG, "io tx param failed");
